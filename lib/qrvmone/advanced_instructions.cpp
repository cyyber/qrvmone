...
template <Opcode Op, qrvmc_status_code CoreFn(StackTop, ExecutionState&) noexcept = core::impl<Op>>
inline qrvmc_status_code impl(AdvancedExecutionState& state) noexcept
{
...
template <Opcode Op,
    qrvmc_status_code CoreFn(StackTop, int64_t&, ExecutionState&) noexcept = core::impl<Op>>
inline qrvmc_status_code impl(AdvancedExecutionState& state) noexcept
{
...
/// Wraps the generic instruction implementation to advanced instruction function signature.
template <qrvmc_status_code InstrFn(AdvancedExecutionState&) noexcept>
const Instruction* op(const Instruction* instr, AdvancedExecutionState& state) noexcept
{
    if (const auto status_code = InstrFn(state); status_code != QRVMC_SUCCESS)
...
const Instruction* opx_beginblock(const Instruction* instr, AdvancedExecutionState& state) noexcept
{
    ...
    if (static_cast<int>(state.stack.size()) < block.stack_req)
        return state.exit(QRVMC_STACK_UNDERFLOW);

    if (static_cast<int>(state.stack.size()) + block.stack_max_growth > StackSpace::limit)
        return state.exit(QRVMC_STACK_OVERFLOW);
...
const Instruction* op_jump(const Instruction*, AdvancedExecutionState& state) noexcept
{
    ...
    if (std::numeric_limits<int>::max() < dst ||
        ...
        return state.exit(QRVMC_BAD_JUMP_DESTINATION);
...
{
    return state.exit(QRVMC_UNDEFINED_INSTRUCTION);
}
...
QRVMC_EXPORT const OpTable& get_op_table(qrvmc_revision rev) noexcept
{
