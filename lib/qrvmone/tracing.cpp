...
std::string get_name(uint8_t opcode)
{
    ...
    return (name != nullptr) ? name : "0x" + qrvmc::hex(opcode);
}
...
class HistogramTracer : public Tracer
{
    ...
    void on_instruction_start(uint32_t pc, const intx::uint<512>* /*stack_top*/, int /*stack_height*/,
...
class InstructionTracer : public Tracer
{
    ...
    void output_stack(const intx::uint<512>* stack_top, int stack_height)
    {
        m_out << R"(,"stack":[)";
        ...
        qrvmc_revision rev, const qrvmc_message& msg, bytes_view code) noexcept override
    {
        ...
        m_out << R"(,"rev":")" << rev << '"';
        m_out << R"(,"static":)" << (((msg.flags & QRVMC_STATIC) != 0) ? "true" : "false");
        m_out << "}\n";
    }
        ...
        int64_t gas, const ExecutionState& state) noexcept override
    {
        ...
        // Full memory can be dumped as qrvmc::hex({state.memory.data(), state.memory.size()}),
    ...
    void on_execution_end(const qrvmc_result& result) noexcept override
    {
        ...
        m_out << R"(,"output":")" << qrvmc::hex({result.output_data, result.output_size}) << '"';
        m_out << "}\n";
