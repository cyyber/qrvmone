...
/// The view/controller for QRVM stack.
class Stack
{
public:
...
struct AdvancedExecutionState : ExecutionState
{
    ...
    AdvancedExecutionState(const qrvmc_message& message, qrvmc_revision revision,
        const qrvmc_host_interface& host_interface, qrvmc_host_context* host_ctx,
    ...
    const Instruction* exit(qrvmc_status_code status_code) noexcept
    {
        status = status_code;
        return nullptr;
    }
    ...
    void reset(const qrvmc_message& message, qrvmc_revision revision,
        const qrvmc_host_interface& host_interface, qrvmc_host_context* host_ctx,
        bytes_view _code) noexcept
    {
...
QRVMC_EXPORT AdvancedCodeAnalysis analyze(qrvmc_revision rev, bytes_view code) noexcept;
