...
CodeAnalysis analyze(qrvmc_revision /*rev*/, bytes_view code)
{
    ...
    const uint512* stack_top, const uint512* stack_bottom) noexcept
{
    ...
    if constexpr (!instr::has_const_gas_cost(Op))
    {
        ...
        if (INTX_UNLIKELY(gas_cost < 0))
            return QRVMC_UNDEFINED_INSTRUCTION;
    }
    ...
    if constexpr (instr::traits[Op].stack_height_required > 0)
    {
        ...
        if (INTX_UNLIKELY(stack_top <= stack_bottom + min_offset))
            return QRVMC_STACK_UNDERFLOW;
    }

    if (INTX_UNLIKELY((gas_left -= gas_cost) < 0))
        return QRVMC_OUT_OF_GAS;

    return QRVMC_SUCCESS;
}
...
#undef ON_OPCODE
        ...
        default:
            state.status = QRVMC_UNDEFINED_INSTRUCTION;
            return gas;
        }
    }
    intx::unreachable();
    ...
    const VM& vm, int64_t gas, ExecutionState& state, const CodeAnalysis& analysis) noexcept
{
    ...
    const auto gas_left = (state.status == QRVMC_SUCCESS || state.status == QRVMC_REVERT) ? gas : 0;
    const auto gas_refund = (state.status == QRVMC_SUCCESS) ? state.gas_refund : 0;

    assert(state.output_size != 0 || state.output_offset == 0);
    const auto result = qrvmc::make_result(state.status, gas_left, gas_refund,
...
qrvmc_result execute(qrvmc_vm* c_vm, const qrvmc_host_interface* host, qrvmc_host_context* ctx,
