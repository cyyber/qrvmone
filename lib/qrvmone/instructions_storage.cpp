...
/// Table of gas cost specification for storage instructions per QRVM revision.
...
constexpr auto sstore_costs = []() noexcept {
    std::array<std::array<StorageStoreCost, QRVMC_STORAGE_MODIFIED_RESTORED + 1>,
    ...
    for (size_t rev = QRVMC_ZOND; rev <= QRVMC_MAX_REVISION; ++rev)
    {
        ...
        e[QRVMC_STORAGE_ASSIGNED] = {c.warm_access, 0};
        e[QRVMC_STORAGE_ADDED] = {c.set, 0};
        e[QRVMC_STORAGE_DELETED] = {c.reset, c.clear};
        e[QRVMC_STORAGE_MODIFIED] = {c.reset, 0};
        e[QRVMC_STORAGE_DELETED_ADDED] = {c.warm_access, static_cast<int16_t>(-c.clear)};
        e[QRVMC_STORAGE_MODIFIED_DELETED] = {c.warm_access, c.clear};
        e[QRVMC_STORAGE_DELETED_RESTORED] = {
            c.warm_access, static_cast<int16_t>(c.reset - c.warm_access - c.clear)};
        e[QRVMC_STORAGE_ADDED_DELETED] = {
            c.warm_access, static_cast<int16_t>(c.set - c.warm_access)};
        e[QRVMC_STORAGE_MODIFIED_RESTORED] = {
...
Result sstore(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    if (state.in_static_mode())
        return {QRVMC_STATIC_MODE_VIOLATION, gas_left};
