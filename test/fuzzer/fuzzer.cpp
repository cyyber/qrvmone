...
inline std::ostream& operator<<(std::ostream& os, const qrvmc_address& addr)
{
...
/// The reference VM: qrvmone Baseline
static auto ref_vm = qrvmc::VM{qrvmc_create_qrvmone()};

static qrvmc::VM external_vms[] = {
    qrvmc::VM{qrvmc_create_qrvmone(), {{"advanced", ""}}},
};


class FuzzHost : public qrvmc::MockedHost
{
public:
...
inline qrvmc::address generate_interesting_address(uint8_t b) noexcept
{
    const auto s = (b >> 6) & 0b11;
    const auto fill = (b >> 5) & 0b1;
    const auto above = (b >> 4) & 0b1;
...
{
    // TODO: If timestamp is -1 Aleth and qrvmone disagrees how to convert it to uint256.
...
fuzz_input populate_input(const uint8_t* data, size_t data_size) noexcept
{
    ...
    // in.rev = (rev_4bits > QRVMC_LATEST_STABLE_REVISION) ? QRVMC_LATEST_STABLE_REVISION :
    //                                                      static_cast<qrvmc_revision>(rev_4bits);
    in.rev = QRVMC_LATEST_STABLE_REVISION;

    // The message king should not matter but this 1 bit was free.
    in.msg.kind = kind_1bit ? QRVMC_CREATE : QRVMC_CALL;

    in.msg.flags = static_1bit ? QRVMC_STATIC : 0;
    ...
    in.host.call_result.status_code = static_cast<qrvmc_status_code>(call_result_status_4bits);
...
inline auto hex(const qrvmc_address& addr) noexcept
{
    return hex({addr.bytes, sizeof(addr)});
}

inline qrvmc_status_code check_and_normalize(qrvmc_status_code status) noexcept
{
    ASSERT(status >= 0);
    return status <= QRVMC_REVERT ? status : QRVMC_FAILURE;
}
    ...
    for (auto& vm : external_vms)
    {
        ...
        if (ref_status != QRVMC_FAILURE)
        {
            ...
            for (size_t i = 0; i < ref_host.recorded_calls.size(); ++i)
            {
                ...
                ASSERT_EQ(qrvmc::address{m1.recipient}, qrvmc::address{m2.recipient});
