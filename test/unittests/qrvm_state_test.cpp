...
/// This file contains QRVM unit tests that access or modify information
...
TEST_P(qrvm, codecopy_combinations)
{
...
TEST_P(qrvm, account_info_homestead)
{
    ...
    EXPECT_GAS_USED(QRVMC_SUCCESS, 123);
    ASSERT_EQ(result.output_size, 1);
    EXPECT_EQ(result.output_data[0], 1);
}

TEST_P(qrvm, selfbalance)
{
    host.accounts[msg.recipient].set_balance(0x0504030201);
...
TEST_P(qrvm, log)
{
    for (auto op : {OP_LOG0, OP_LOG1, OP_LOG2, OP_LOG3, OP_LOG4})
    {
        ...
        execute(code);
        EXPECT_GAS_USED(QRVMC_SUCCESS, 421 + n * 375);
...
TEST_P(qrvm, log_data_cost)
{
    for (auto op : {OP_LOG0, OP_LOG1, OP_LOG2, OP_LOG3, OP_LOG4})
    {
        ...
        EXPECT_EQ(host.recorded_logs.size(), 0) << qrvmone::instr::traits[op].name;
        host.recorded_logs.clear();
    }
}
...
TEST_P(qrvm, extcodecopy_big_index)
{
...
TEST_P(qrvm, codecopy_memory_cost)
{
...
TEST_P(qrvm, extcodecopy_memory_cost)
{
...
TEST_P(qrvm, extcodecopy_buffer_overflow)
{
