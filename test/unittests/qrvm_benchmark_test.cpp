...
/// This file contains QRVM code generators for micro benchmarks,
...
TEST_P(qrvm, grow_memory_with_mload)
{
...
TEST_P(qrvm, grow_memory_with_mstore)
{
    const auto code = calldataload(0) + push(0) +
...
TEST_P(qrvm, jump_around)
{
    ...
    execute(code);
    EXPECT_GAS_USED(QRVMC_SUCCESS, int64_t{(1 + 3 + 8) * num_jumps + 1});
}

TEST_P(qrvm, signextend_bench)
{
    ...
    execute(code);
    EXPECT_GAS_USED(QRVMC_SUCCESS, 8 * num_instr + 15 * 3 + 21);
    EXPECT_OUTPUT_INT(0);
}
