...
TEST_P(qrvm, memory_and_not)
{
    ...
    EXPECT_GAS_USED(QRVMC_SUCCESS, 42);
...
TEST_P(qrvm, memory_big_allocation)
{
...
TEST_P(qrvm, memory_grow_mstore8)
{
...
TEST_P(qrvm, mstore8_memory_cost)
{
...
TEST_P(qrvm, keccak256_memory_cost)
{
...
TEST_P(qrvm, calldatacopy_memory_cost)
{
    const auto code = calldatacopy(0, 0, 1);
    execute(18, code);
    EXPECT_GAS_USED(QRVMC_SUCCESS, 18);
}
...
TEST_P(qrvm, memory_access)
{
    ...
    for (const auto& p : memory_access_test_cases)
    {
        for (auto& t : memory_access_opcodes)
        {
            ...
            {
                EXPECT_EQ(result.status_code, (t.opcode == OP_REVERT) ? QRVMC_REVERT : QRVMC_SUCCESS)
                    << case_descr;
            ...
            else
            {
                if (t.opcode == OP_RETURNDATACOPY)
                {
                    ...
                    EXPECT_TRUE(result.status_code == QRVMC_OUT_OF_GAS ||
                                result.status_code == QRVMC_INVALID_MEMORY_ACCESS);
                }
                else
                {
                    EXPECT_EQ(result.status_code, QRVMC_OUT_OF_GAS) << case_descr;
                }
