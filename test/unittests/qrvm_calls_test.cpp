...
/// This file contains QRVM unit tests that perform any kind of calls.
...
TEST_P(qrvm, delegatecall_static)
{
    ...
    EXPECT_EQ(call_msg.flags, uint32_t{QRVMC_STATIC});
    EXPECT_GAS_USED(QRVMC_SUCCESS, 119);
}
...
TEST_P(qrvm, delegatecall_oog_depth_limit)
{
    ...
    execute(code);
    EXPECT_GAS_USED(QRVMC_SUCCESS, 133);
    EXPECT_OUTPUT_INT(0);
...
TEST_P(qrvm, create2_salt_cost)
{
    ...
    EXPECT_EQ(result.status_code, QRVMC_SUCCESS);
    ASSERT_EQ(host.recorded_calls.size(), 1);
    EXPECT_EQ(host.recorded_calls.back().kind, QRVMC_CREATE2);
...
TEST_P(qrvm, create_failure)
{
    ...
    for (auto op : {OP_CREATE, OP_CREATE2})
    {
        ...
        host.call_result.status_code = QRVMC_REVERT;
        execute(code);
        EXPECT_EQ(result.status_code, QRVMC_SUCCESS);
        EXPECT_OUTPUT_INT(0);
        ASSERT_EQ(host.recorded_calls.size(), 1);
...
TEST_P(qrvm, call_value_zero_to_nonexistent_account)
{
...
TEST_P(qrvm, delegatecall_then_oog)
{
    // Performs a CALL then OOG in the same code block.
    auto call_dst = qrvmc_address{};
    call_dst.bytes[63] = 0xaa;
...
TEST_P(qrvm, staticcall_then_oog)
{
...
TEST_P(qrvm, call_recipient_and_code_address)
{
    ...
    for (auto op : {OP_CALL, OP_DELEGATECALL, OP_STATICCALL})
    {
        ...
        execute(100000, code);
        EXPECT_GAS_USED(QRVMC_SUCCESS, (op == OP_CALL) ? 2621 : 121);
        ASSERT_EQ(host.recorded_calls.size(), 1);
...
// TODO(now.youtrack.cloud/issue/TE-13)
/*
TEST_P(qrvm, create_oog_after)
{
    rev = QRVMC_CONSTANTINOPLE;
    for (auto op : {OP_CREATE, OP_CREATE2})
    {
...
TEST_P(qrvm, returndatasize_before_call)
{
...
TEST_P(qrvm, returndatasize)
{
    ...
    host.call_result.status_code = QRVMC_INTERNAL_ERROR;
    execute(code);
    EXPECT_EQ(gas_used, 135);
    ASSERT_EQ(result.output_size, 1);
    EXPECT_EQ(result.output_data[0], 0);
...
TEST_P(qrvm, returndatacopy_cost)
{
...
TEST_P(qrvm, returndatacopy_outofrange)
{
    auto call_output = uint8_t{};
    host.call_result.output_data = &call_output;
    host.call_result.output_size = sizeof(call_output);
