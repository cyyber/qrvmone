...
int64_t compute_tx_data_cost(qrvmc_revision /*rev*/, bytes_view data) noexcept
{
...
int64_t compute_tx_intrinsic_cost(qrvmc_revision rev, const Transaction& tx) noexcept
{
...
/// Validates transaction and computes its execution gas limit (the amount of gas provided to QRVM).
...
std::variant<int64_t, std::error_code> validate_transaction(const Account& sender_acc,
    const BlockInfo& block, const Transaction& tx, qrvmc_revision rev) noexcept
{
...
qrvmc_message build_message(const Transaction& tx, int64_t execution_gas_limit) noexcept
{
    const auto recipient = tx.to.has_value() ? *tx.to : qrvmc::address{};
    return {
        tx.to.has_value() ? QRVMC_CALL : QRVMC_CREATE,
        0,
        0,
        execution_gas_limit,
        recipient,
...
void finalize(State& state, qrvmc_revision /*rev*/, std::span<Withdrawal> withdrawals)
{
...
std::variant<TransactionReceipt, std::error_code> transition(
    State& state, const BlockInfo& block, const Transaction& tx, qrvmc_revision rev, qrvmc::VM& vm)
{
    ...
    sender_acc.access_status = QRVMC_ACCESS_WARM;  // Tx sender is always warm.
    if (tx.to.has_value())
    ...
    for (const auto& [a, storage_keys] : tx.access_list)
    {
        ...
        for (const auto& key : storage_keys)
            storage[key].access_status = QRVMC_ACCESS_WARM;
    }
...
[[nodiscard]] bytes rlp_encode(const TransactionReceipt& receipt)
{
    ...
    return prefix + rlp::encode_tuple(receipt.status == QRVMC_SUCCESS,
